        ...
        def calc_subs():
            for i in range(len(data_money_subs)):
                if data_money_subs['duration_min'].iloc[i] > 45:
                    data_money_subs[('price_per_trip','')].iloc[i] = data_money_subs[('price_per_trip','')].iloc[i] + math.ceil((data_money_subs['duration_min'].iloc[i]-45)/15) * 3
...
def Task2():
    ...
    df = pd.read_csv('Task 2/task2_data.csv')
